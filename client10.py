...
client_id = 10  # クライアントごとに異なるIDを設定する
...
# AUPRCを最適化するためのスコアラーとグリッドサーチの設定
auprc_scorer = make_scorer(average_precision_score)
grid_search_auprc = GridSearchCV(model, param_grid, scoring=auprc_scorer, cv=5, n_jobs=-1)
grid_search_auprc.fit(X_scaled, y)
best_model_auprc = grid_search_auprc.best_estimator_

# 最適なパラメータとスコアを表示
print(f"Best C value for AUPRC: {grid_search_auprc.best_params_['C']}")
print(f"Best AUPRC score: {grid_search_auprc.best_score_}")

# Flowerクライアントの定義
class FederatedClient(fl.client.NumPyClient):
    def get_parameters(self, config):
        return [best_model_auprc.coef_, best_model_auprc.intercept_] if best_model_auprc.fit_intercept else [best_model_auprc.coef_]

    def set_parameters(self, parameters):
        best_model_auprc.coef_ = parameters[0]
        if best_model_auprc.fit_intercept:
            best_model_auprc.intercept_ = parameters[1]

    def fit(self, parameters, config):
        self.set_parameters(parameters)
        best_model_auprc.fit(X_scaled, y)
        return self.get_parameters({}), len(X_scaled), {}

    def evaluate(self, parameters, config):
        self.set_parameters(parameters)
        y_pred_proba = best_model_auprc.predict_proba(X_scaled)
        loss = log_loss(y, y_pred_proba)
        auprc = average_precision_score(y, y_pred_proba[:, 1])
        return loss, len(X_scaled), {"loss": loss, "auprc": auprc}
